...
//  stick
//
//  Created by xena  ayala on 4/2/19.
...
void draw_upper_torso() {
    draw_limb(1, upper_torso_angle, 0.0f, 0.0f, '5', '5', 0.0f, 3.0f);
...
{ draw_middle_limb(direction, angle, 3.0f, 3.0f, '2', '1', upper_middle_arm_length, 0.0f); }

void draw_wrist(int direction, float angle)
{ draw_middle_limb(direction, angle, upper_middle_arm_length, 0.0f, '2', '1', upper_middle_arm_length, 0.0f); }

void draw_hand(int direction, float angle)
{ draw_limb(direction, angle, upper_middle_arm_length, 0.0f, '2', '1', 0.0f, lower_arm_length); }
...
{ draw_middle_limb(direction, angle, 2.0f, -3.0f, '4', '3', 0.0f, -upper_middle_leg_length); }

void draw_ankle(int direction, float angle)
{ draw_middle_limb(direction, angle, 0.0f, -upper_middle_leg_length, '4', '3', 0.0f, -upper_middle_leg_length); }

void draw_foot(int direction, float angle)
{ draw_limb(direction, angle, 0.0f, -upper_middle_leg_length, '4', '3', lower_leg_length, 0.0f); }
...
void processKeys(unsigned char key, int x, int y) {
    ...
    if (key == 27) exit(0);
            ...
            //Move head and torso Left and right
        case 'L':
        case 'l':
            switch(limb){
                case '5': upper_torso_angle += magnitude; break;
                case '6': head_angle += magnitude;          break;
            }
        ...
        //T and Y move upperbones clockwise and counterclockwise
        //G and H move lowerbones clockwise and counterclockwise
        ...
        case 'y':
            switch(limb) {
                ...
                case '1': l_shoulder_angle += magnitude;  break;
                case '2': r_shoulder_angle += magnitude;  break;
                case '3': l_hip_angle += magnitude;          break;
                case '4': r_hip_angle += magnitude;          break;
            }
            break;
        case 'T':
        case 't':
        ...
        case 'h':
            switch(limb) {
                case '1': l_elbow_angle += magnitude; break;
                case '2': r_elbow_angle += magnitude; break;
                case '3': l_knee_angle += magnitude;  break;
                case '4': r_knee_angle += magnitude;  break;
            }
            break;
        case 'G':
        case 'g':
